...
public class DiscordHtmlTranscripts {
    ...
    public Collection<? extends FileUpload> createTranscript(TextChannel channel, TextChannel transcriptChannel) throws IOException {
        createTranscript(channel,transcriptChannel, "transcript.html");
        return null;
    }

    public void createTranscript(MessageChannel channel,TextChannel transcriptChannel , String fileName) throws IOException {
        fileUpload a = new fileUpload(generateFromMessages(channel.getIterableHistory().stream().collect(Collectors.toList())), fileName);
        transcriptChannel.sendFiles(a).queue();
        // channel.sendFiles(generateFromMessages(channel.getIterableHistory().stream().collect(Collectors.toList())), fileName != null ? fileName : "transcript.html").queue();
    }

    public Collection<? extends FileUpload> createDMTranscript(TextChannel channel, PrivateChannel transcriptChannel) throws IOException {
        createDMTranscript(channel,transcriptChannel, "transcript.html");
        return null;
    }
    public void createDMTranscript(MessageChannel channel, PrivateChannel transcriptChannel, String fileName) throws IOException {
        fileUpload a = new fileUpload(generateFromMessages(channel.getIterableHistory().stream().collect(Collectors.toList())), fileName);
        transcriptChannel.sendFiles(a).queue();
    }

    public InputStream generateFromMessages(Collection<Message> messages) throws IOException {
        File htmlTemplate = new File("C:\\Users\\Daniel Kenyon\\IdeaProjects\\DevHubBot\\target\\classes\\template.html");
        if (messages.isEmpty()) {
